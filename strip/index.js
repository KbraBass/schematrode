...
const getNamespaces = (message, msgType) => {
  const ret = {
    cac: '', // We are going for empty ones per default if there is no namespace
    cbc: ''
  };
  Object.entries(message[msgType].$).some(([key, value]) => {
    // We have the Namespace declarations here... xmlns, and the CAC and CBC
    if (key.includes(':') && value.includes('CommonAggregateComponents')) {
      // This is our CAC as "xmlns:cac"
      ret.cac = `${key.split(':')[1]}:`;
    }
    if (key.includes(':') && value.includes('CommonBasicComponents')) {
      // This is our CBC, as "xmlns:cbc"
      ret.cbc = `${key.split(':')[1]}:`;
    }
    return !!(ret.cac && ret.cbc); // Break when we have both values
  });
  return ret;
};


const generateENRules = async () => {
  const schematron = await fetch('https://raw.githubusercontent.com/OpenPEPPOL/peppol-bis-invoice-3/refs/heads/master/rules/sch/CEN-EN16931-UBL.sch').then((response) => response.text());
  const jssch = await convertUblXmlToUblJson(schematron);
  const rules = [];
  jssch.schema.pattern.reduce((r, a) => {
    r?.rule?.forEach((p) => rules.push({
      context: p.$.context,
      assert: p.assert[0]._,
      flag: p.$.flag || p.assert[0].$.flag, // Flag can be on rule, or assert
      test: p.assert[0].$.test
    }));
    a?.rule?.forEach((p) => rules.push({
      context: p.$.context,
      assert: p.assert[0]._,
      flag: p.$.flag || p.assert[0].$.flag, // Flag can be on rule, or assert
      test: p.assert[0].$.test
...
<assert id="BR-CO-10" flag="fatal" test="(xs:decimal(cbc:LineExtensionAmount) = xs:decimal(round(sum(//(cac:InvoiceLine|cac:CreditNoteLine)/xs:decimal(cbc:LineExtensionAmount)) * 10 * 10) div 100))">
...
const stripPeppolItems = async (j, msgType, enRules) => {

  enRules.forEach((r) => {
    if (/.*(ubl:Invoice|cn:CreditNote).+\|.*(ubl:Invoice|cn:CreditNote)/.test(r.context) || /.*(cac:InvoiceLine|cac:CreditNoteLine).+\|.*(cac:InvoiceLine|cac:CreditNoteLine)/.test(r.context)) {
      // We might have two option (OR) for Invoice and CreditNote, we'll remove the not needed one
      r.context = (r.context.split('|').at(0)).trim().replace(/:Invoice|:CreditNote/, `:${msgType}`);
    }
    const mtR = new RegExp(`^\/ubl:${msgType}`);
    if (mtR.test(r.context)) { // A path starting with a slash and message type is for a specific path only
      //console.log('===>', r.context, r.test);
      let condition;
      if (/\[(.+)\]/.test(r.context)) {
        // We have a condition... e.g. /ubl:Invoice/cac:AllowanceCharge[cbc:ChargeIndicator = false()]
        condition = r.context.split('[').at(1).slice(0, -1);
        r.context = r.context.split('[').at(0);
        //console.log('cccc>>>>', r.context);
      }
      const p = r.context.split('/');
      let e;
      p.forEach((c) => {
        if (c) {
          e = e?.[c.replace('ubl:', '').replace(/cac:/g, 'cac_').replace(/cbc:/g, 'cbc_')] || e?.[0]?.[c.replace('ubl:', '').replace(/cac:/g, 'cac_').replace(/cbc:/g, 'cbc_')] || j[c.replace('ubl:', '').replace(/cac:/g, 'cac_').replace(/cbc:/g, 'cbc_')];
        }
      });
      //console.log('condition', condition);
      //console.log('eeeee', e);

    } else if (r.context.includes('InvoiceLine')) {
      //console.log('InvoiceLine ===>', r.context, r.test);
    }
  });
...
(async () => {
  ...
  const enRules = await generateENRules();

  console.time('ReadFile');
  const invoice = await fs.readFileSync('./Large_Invoice_sample2.xml', 'utf-8');
  console.timeEnd('ReadFile');

  const js = await convertUblXmlToUblJson(stripPeppolAttachments(invoice));
  const msgType = Object.keys(js)[0];
  const ns = getNamespaces(js, msgType);

  // To make our life easier...
  const regExpCac = new RegExp(`${ns.cac}`, 'g');
  const regExpCbc = new RegExp(`${ns.cbc}`, 'g');
  const message = invoice.replace(regExpCac, 'cac_').replace(regExpCbc, 'cbc_');
  // console.log('byteSize XML:', byteSize(message));
  console.time('XML2JS');
  j = await convertUblXmlToUblJson(message);
  // console.log(JSON.stringify(j, null, 2));
  console.timeEnd('XML2JS');

  // Add line items to the Peppol XML (if needing more)
  //j[msgType].cac_InvoiceLine = fillArr(j[msgType].cac_InvoiceLine, 60000);


  return stripPeppolItems(j, msgType, enRules);
